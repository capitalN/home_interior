...
export const add_to_shop = (product) => async (dispatch) => {
  try {
    let res = await axios.post("http://localhost:4040/furniture", product);
...
export const update_shop = (product) => async (dispatch) => {
  ...
  let res = await axios.put(
    `http://localhost:4040/furniture/${product.id}`,
    product
  );
...
export const delete_from_shop = (id) => async (dispatch) => {
  let res = await axios.delete(`http://localhost:4040/furniture/${id}`);
